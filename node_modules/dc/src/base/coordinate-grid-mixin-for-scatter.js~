 ...
 * @mixes MarginMixin
 */
export class CoordinateGridMixinForScatter extends ColorMixin(MarginMixin) {
    constructor () {
        super();
        ...
        this._y = undefined;
        this._origY = undefined; // Will hold original scale in case of zoom
        this._yOriginalDomain = undefined;
        this._yAxis = null;
    ...
    y (yScale) {
        ...
        this._y = yScale;
       this._yOriginalDomain = this._y.domain();
       this.rescale();
        return this;
    }

    yOriginalDomain () {
      return this._yOriginalDomain;
  }
...
2
    ...
    _zoomHandler (xnewDomain, ynewDomain, noRaiseEvents) {
        let xdomFilter, ydomFilter;

        if (this._hasRangeSelected(xnewDomain)) {
            this.x().domain(xnewDomain);
            xdomFilter = filters.RangedFilter(xnewDomain[0], xnewDomain[1]);
        } else {
            this.x().domain(this._xOriginalDomain);
            xdomFilter = null;
        }

        if (this._hasRangeSelected(ynewDomain)) {
            this.y().domain(ynewDomain);
            ydomFilter = filters.RangedFilter(ynewDomain[0], ynewDomain[1]);
        } else {
            this.y().domain(this._yOriginalDomain);
            ydomFilter = null;
        }

        this.replaceFilter(xdomFilter);
        this.replaceFilter(ydomFilter);
        this.rescale();
        this.redraw();
    ...
    _domainToZoomTransform (xnewDomain, xorigDomain, xScale, ynewDomain, yorigDomain, yScale) {
        ...
        const yt = -1 * yScale(ynewDomain[0]);

        return zoomIdentity.scale(yk).translate(xt, yt);
    }
    ...
    _onZoom () {
        ...
        const xnewDomain = event.transform.rescaleX(this._origX).domain();
        const ynewDomain = event.transform.rescaleY(this._origY).domain();
        this.focus(xnewDomain, ynewDomain, false);
    }
     ...
     * @return {undefined}
     */
    focus (xrange, yrange, noRaiseEvents) {
        if (this._zoomOutRestrict) {
            // ensure range is within self._xOriginalDomain
            xrange = this._checkExtents(xrange, this._xOriginalDomain);
            yrange = this._checkExtents(yrange, this._yOriginalDomain);

            // If it has an associated range chart ensure range is within domain of that rangeChart
            if (this._rangeChart) {
                xrange = this._checkExtents(xrange, this._rangeChart.x().domain());
                yrange = this._checkExtents(yrange, this._rangeChart.y().domain());
            }
        }

        this._zoomHandler(xrange, yrange, noRaiseEvents);
        this._updateD3zoomTransform();
    }
    ...
    refocused () {
        return !utils.arraysEqual(this.y().domain(), this._yOriginalDomain);
    }
        ...
        this.on('filtered.dcjs-range-chart', chart => {
            if (!chart.filter()) {
                events.trigger(() => {
                    ...
                    this._focusChart.y().domain(this._focusChart.yOriginalDomain(), true);
                });
            } else if (!utils.arraysEqual(chart.filter(), this._focusChart.filter())) {
                events.trigger(() => {
                    this._focusChart.focus(chart.filter(), chart.filter(), true);
                });
            }
        });
        return this;
